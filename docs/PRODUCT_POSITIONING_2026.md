...
| V2.3 PRISMA-trAIce readiness | 已在 `literature-screening-v2.2/` 建立 AI mode、AI usage registry、AI suggestion log、人工复核 trace 字段和 `PRISMA_TRAICE_REPORT.md`；仅使用本地 mock pathway，不接真实 AI provider |
...
3. V2.3 readiness 已完成数据模型、mock suggestion review flow 和透明报告；下一步仍不接默认云模型，先做发布收口和 V2.4 质量评价结构化。
