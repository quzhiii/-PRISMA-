...
V2.6 foundation slice current implementation:
...
| `reviewed_at` | 人工接受、拒绝或改写建议的时间 |
| `human_edited_decision` | 人工改写后的明确 decision |
| `human_edited_exclusion_reason` | 人工改写为 exclude 时的标准化排除理由 |
| `human_edited_original_exclusion_reason` | 用户界面中选择或输入的原始排除理由 |
| `review_note` | 人工复核动作说明 |
| `prisma_count_boundary` | `linked_human_screening_decision_required_for_counts` 或 `advisory_log_only_not_counted_directly` |
| `created_at` | 时间 |
