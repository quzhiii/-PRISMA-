...
一个强大的文献筛选和 PRISMA 2020 流程图生成工具，帮助研究者高效完成系统综述和荟萃分析的文献筛选工作。**v1.6 工程化增强版：进度可视化、容错机制、去重可解释。**

##  核心功能

###  v1.6 工程化增强版（最新）
- ⚡ **可视化进度增强**：实时ETA估算（剩余时间）、解析/写入速度显示、暂停/继续按钮
- 🛡️ **导入容错机制**：详细错误报告（文件名+行号+错误类型）、成功率统计、跳过问题记录继续导入
- 🔍 **去重策略可解释**：展示DOI优先级和标题归一化规则、去重统计、导出去重详情报告
...
### v1.6 工程化增强使用指南
- ⚡ **进度可视化**：导入时查看剩余时间、解析速度、写入速度，大文件导入心中有数
- ⏸️ **暂停/继续**：导入过程中可随时暂停，查看其他内容后继续
- 🛡️ **容错导入**：即使文件中有格式错误，也能跳过问题记录继续导入，最后查看详细错误报告
- 🔍 **去重透明**：点击"查看去重策略说明"了解DOI和标题去重规则，导出去重详情审计
- 📊 **错误报告**：导入完成后查看哪些记录失败、具体原因、失败位置，方便修复源文件
...
-  **错误报告**：v1.6支持导出错误和去重详情，方便调试和审计

##  v1.6 新增功能详解

### 1. 可视化进度增强 ⚡
**问题**：之前导入3万篇文献时，用户不知道还需要等多久，容易焦虑。

**解决方案**：
- 实时显示剩余时间（秒/分钟/小时格式）
- 显示解析速度和写入速度（条/秒）
- 添加暂停/继续按钮，用户可中途停止查看其他内容
- 平滑的进度条动画（0.3s transition）

**使用场景**：导入2万篇文献时，看到"剩余 2分30秒 | 解析: 450条/秒 | 写入: 380条/秒"

### 2. 导入容错机制 🛡️
**问题**：一个文件中如果有1条记录格式错误，整个文件就会导入失败。

**解决方案**：
- errorTracker系统收集所有错误和警告
- 详细记录：文件名、行号、错误类型、具体内容
- 跳过问题记录，继续导入其他正常记录
- 导入完成后显示成功率和错误摘要
- 支持导出错误报告为JSON

**使用场景**：导入10000条记录，其中50条有格式问题，系统显示"成功率: 99.5% (9950/10000 条记录)，有50个错误"，点击查看详细错误列表。

### 3. 去重策略可解释 🔍
**问题**：用户不理解为什么某些文献被判定为重复，缺乏透明度。

**解决方案**：
- 展示去重策略：DOI精确匹配 → 标题归一化匹配
- 标题归一化规则说明：移除标点、空格、转小写
- 记录每条文献的去重决策（DOI重复/标题重复/保留）
- 去重统计：通过DOI去重X篇，通过标题去重Y篇
- 导出去重详情报告（JSON格式）

**使用场景**：点击"查看去重策略说明"，看到详细规则和示例，理解为什么"The Effect of Acupuncture!"和"the effect of acupuncture"被判定为重复。
...
```
┌─────────────────┐
│   index.html    │  UI 层：步骤向导 + 虚拟滚动列表 + 进度可视化
└────────┬────────┘
         │
┌────────▼────────┐
│     app.js      │  业务逻辑层：规则引擎 + 流程控制 + 容错系统
│                 │  • progressTracker（进度追踪）
│                 │  • errorTracker（错误追踪）
│                 │  • dedupExplanations（去重解释）
└────────┬────────┘
         │
    ┌────┴────┐
    │         │
...
- **app.js**：主业务逻辑，规则引擎、PRISMA 生成、Excel 导出
  - **progressTracker**：实时进度追踪（ETA估算、速度统计、暂停/继续）
  - **errorTracker**：错误收集系统（错误/警告分类、成功率计算、详细报告）
  - **dedupExplanations**：去重决策记录（策略追踪、统计分析、可解释性）
...
- **虚拟滚动**：分页渲染技术，降低内存占用
- **流式处理**：边解析边存储，避免内存溢出
- **容错机制**：跳过错误记录继续处理，详细错误追踪
- **可视化增强**：实时ETA计算、速度监控、暂停/恢复控制
