...
## 🔥 v1.7 新特性 (2026-03 Verified Update)

本次已针对最新用户反馈完成一轮 **v1.7.x 稳定性修复与回归验证**。除了原有的关键词优先级和去重增强外，当前版本还重点修复了导入、协作模式和去重后流程中断等问题。

### 本轮已修复问题（已验证）

#### 1. 🧾 PubMed `.nbib` 导入支持补全
- 上传控件现在明确支持 `.nbib`
- 解析链路已接通：上传校验 → 主线程解析 → Worker 解析
- `.nbib` 会按 PubMed / RIS 兼容路径解析
- 已用真实 `pubmed-sample.nbib` fixture 回归验证

#### 2. 👥 单人 / 双人模式切换修复
- 主应用现在会读取 `prisma_user_session`
- 可正确区分：
  - `single`
  - `dual-main`
  - `dual-secondary`
- 协作相关按钮（项目导出、分歧处理、状态查看）已按角色做 gating
- `login.html` 登录后会正确写入 `sessionStorage`

#### 3. 🔄 去重后无法继续的问题修复
- 修复了 `v1.7-core-patch.js` 中脆弱的 Step 2 / Step 4 集成逻辑
- 去重详情弹窗与“仅去重导出”功能已补强
- 实测流程可从：
  - 上传
  - 配置规则
