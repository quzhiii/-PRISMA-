# PRISMA 文献筛选助手 v1.5 - 大规模文献处理版

## 核心改造目标
将文献分析容量从 **1000-5000 篇扩展到 3 万篇**，同时保持响应流畅。
...
#### 1. **数据层：IndexedDB 替代 localStorage** 📊
- **文件**：`db-worker.js`
- **改造内容**：
  - 使用浏览器原生 IndexedDB（支持无限存储）
  - 所有数据库操作在 Web Worker 中执行
  - 支持索引查询、范围查询、事务处理
  - 自动分批插入（避免一次性塞入 30000 条数据导致卡顿）

- **核心接口**：
  ```javascript
  await dbWorker.send({
    type: 'IMPORT_RECORDS',
    data: { records: [...], batchSize: 500 }
  });
  
  const pagedData = await dbWorker.send({
    type: 'GET_PAGED',
    data: { pageNum: 0, pageSize: 100 }
  });
  ```

#### 2. **计算层：Web Worker 隔离处理** ⚙️
- **文件**：`parser-worker.js`
- **改造内容**：
  - 文件解析（CSV/RIS/BibTeX 等）在后台线程进行
  - 流式处理大文件（每 1000 行汇报一次进度）
  - 去重逻辑迁移到 Worker（减少主线程压力）
  - 数据标准化在此层完成

#### 3. **UI 层：虚拟滚动列表** 🎯
- **文件**：`virtual-list.js`
- **改造内容**：
  - 只在 DOM 中渲染可见的 100 条项（而非 30000 条）
  - 高频滚动时自动预加载相邻页面
  - 分页获取数据，按需加载
  - 支持快速定位和搜索
...
| 文件 | 用途 | 状态 |
|------|------|------|
| `db-worker.js` | IndexedDB 数据库层 | ✅ 新增 |
| `parser-worker.js` | 文件解析和去重 | ✅ 新增 |
| `virtual-list.js` | 虚拟滚动列表 | ✅ 新增 |
| `app.js` | 主应用逻辑 | 🔄 开发中 |
| `index.html` | UI 页面 | 🔄 开发中 |
| `style.css` | 样式表 | 🔄 开发中 |
...
- [x] P0.1: IndexedDB 数据层
- [x] P0.2: Web Worker 解析层
- [x] P0.3: 虚拟滚动列表
- [ ] P1.1: 分阶段检查存储
- [ ] P1.2: 流式解析大文件
- [ ] P2.1: 主动学习建议
